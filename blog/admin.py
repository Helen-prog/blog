...
class BlogAdmin(admin.ModelAdmin):
    ...
    fields = ('title', 'slug', 'cat', 'content', 'photo',
              'get_html_photo_1', 'is_published', 'time_created', 'time_update')
    readonly_fields = ('get_html_photo_1', 'time_created', 'time_update')

    def get_html_photo(self, objects):
        if objects.photo:
            return mark_safe(f"<img src='{objects.photo.url}' width='70'>")

    def get_html_photo_1(self, objects):
        if objects.photo:
            return mark_safe(f"<img src='{objects.photo.url}' width='200'>")
